<?php
    ...
    if (mysqli_num_rows($result) > 0){ // IF there are more than 0 users in query result,
        ...
        //Successful status code, token, authentication value and username is sent to Ajax
        echo json_encode(array("statusCode"=>200, "token"=>$token, "auth"=>$authorized, "username"=>$uName));
    
    }else{ // IF there are NO user in results
        echo json_encode(array("statusCode"=>422)); //Unprocessable Entity error code for wrong input is sent 
    }
