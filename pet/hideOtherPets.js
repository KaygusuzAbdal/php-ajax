...
function hideOtherPets(username,auth) {
    ...
    var petItems = document.querySelectorAll(".petItem");
    if (auth == "true") { //if user is admin,all pets are shown
        ...
        petItems.forEach(function (item) {
            ...
            if (item.getElementsByTagName("td")[0].innerHTML.includes(username)) {
                item.style.display = "table-row"; //elements are shown
            }
            else { // if username doesn't match with the searched user name
