function startEditPet(target: HTMLElement) {
    //definitions
    const uName: string = target.dataset.uname;
    const pName: string = target.dataset.pname;
    const pkey: string = uName+"_"+pName;
    //User name is written on the username input part in 'Edit Pet' part
    (petEdit.elements.namedItem("username") as HTMLFormElement).value = uName;
        ...
        if(this.readyState == 4 && this.status == 200){
            ...
            data.forEach(function(i){
                if(i.pkey == pkey){  //if pkey value coming from pet info that we retrieved from data, matches with the pkey value we want to do operation
                    ...
                    (petEdit.elements.namedItem("username") as HTMLFormElement).value = i.uName;
                    (petEdit.elements.namedItem("petname") as HTMLFormElement).value = i.pName;
                    (petEdit.elements.namedItem("pettype") as HTMLFormElement).value = i.pType;
